...
starting_tiers = [2, 2, 0, 0]
tiers_to_beat = [5, 5, 1, 1]
	...
	for instability in range(MAX_INSTABILITY - 1, -1, -1):
		...
		for tiers in itertools.product(*[tier_iterator] * 4):
			...
			for upgrade_position, tier in enumerate(tiers):
				...
				guardian_payoff = (1 - max(fchance - 0.25, 0)) * expected_payoff[(instability+5,) + tuple(new_tiers)]
				stability_payoff = (1 - fchance) * np.mean(expected_payoff[(slice(instability+2, instability+6),) + tuple(new_tiers)])
				guardian_action = (guardian_payoff, f'G {upgrade_position}')
				stability_action = (stability_payoff, f'S {upgrade_position}')
				best_action = max(best_action, guardian_action)
				best_action = max(best_action, stability_action)

			expected_payoff[key], best_actions[key] = best_action
